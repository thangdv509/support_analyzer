...
```
Crisp API → fetch_chats() → _grade_one() → Google Sheets
                                         → MongoDB (grading + sumtag)
```
...
### 6. export_to_gsheet(results, date_str)

**Lưu vào:** Google Sheet `"QA Report"` (sheet cố định trong spreadsheet GOOGLE_SHEET_ID).

**Làm gì:**
- Đọc sheet hiện có, dedup theo Crisp URL (cột B)
- Xóa summary cũ bên dưới data (nếu có)
- Append rows mới: date, URL, app, operator, điểm tổng, 14 điểm thành phần, comment, resolved status
- Tính lại bảng summary agent (avg score, total chats) cho **toàn bộ data** trong sheet
- Ghi summary xuống bên dưới data, cách 1 dòng trống
- Format: header xanh đậm, màu điểm (đỏ <7, vàng <9, xanh ≥9), highlight ô bị trừ điểm (cam nhạt), note justification vào từng ô

---

### 7. _grade_and_export(chats, date_str)

**Làm gì:**
- Chạy `_grade_one` song song với `ThreadPoolExecutor(max_workers=5)`
- Lưu kết quả ra file `./report/qa_report_YYYYMMDD.json`
- Gọi `export_to_gsheet()`
...
run_daily_job()
  ...
  ├── _export_to_support_analyzer()   ← Google Sheets "Support Analyzer"
...
### 2. _export_to_support_analyzer(new_results)

**Lưu vào:** Google Sheet `"Support Analyzer"` (khác với `"QA Report"` của `analyzer_v2`).

**Làm gì:**
- Retry 5 lần nếu Google API lỗi 503/500 (exponential backoff)
- Đọc sheet hiện có, dedup theo Crisp URL
- **Xóa summary cũ** trước khi append (tránh append vào sau summary thay vì sau data)
- Append rows mới với cùng format với QA Report
- Tính lại bảng summary từ **toàn bộ data** trong sheet (không chỉ ngày hôm nay)
- Xóa vùng sau data cuối, ghi summary mới vào đúng vị trí
- Format tương tự QA Report, thêm bước reset background trắng cho rows mới (tránh kế thừa màu từ summary cũ)

---

### 3. _generate_summary_and_tags(transcript)
...
| Sheet | File | Nội dung |
|-------|------|----------|
| `QA Report` | `analyzer_v2.py` | Chạy thủ công, cộng dồn theo thời gian |
| `Support Analyzer` | `scheduler.py` | Chạy tự động 9h sáng, cùng format |

Cả hai sheet đều: dedup theo URL, append-only data, tính lại summary agent từ toàn bộ data sau mỗi lần chạy.
...
| `GOOGLE_SHEET_ID` | analyzer_v2, scheduler | ID của Google Spreadsheet |
| `GOOGLE_CREDENTIALS_PATH` | analyzer_v2, scheduler | Path tới service account JSON |
...
python analyzer_v2.py --date 2026-05-04

# Chấm khoảng ngày
python analyzer_v2.py --from 2026-05-01 --to 2026-05-04

# Chấm lại từ file JSON đã có
python analyzer_v2.py --regrade ./report/qa_report_20260504.json

# Scheduler: chấm ngay hôm qua (test)
python scheduler.py --now

# Scheduler: chấm ngày cụ thể rồi thoát
python scheduler.py --date 2026-05-04
