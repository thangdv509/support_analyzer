#!/bin/bash
# Khởi động webhook receiver + ngrok tunnel để mở cổng public trên server
...
#
# Dùng:
#   chmod +x webhook_receiver/start_ngrok.sh
#   ./webhook_receiver/start_ngrok.sh [--port 8090]
#
...
cd "$PROJECT_DIR"
"$VENV_PYTHON" -m uvicorn webhook_receiver.main:app --host 0.0.0.0 --port "$PORT" &
SERVER_PID=$!
...
if [ -n "$NGROK_URL" ]; then
    echo ""
    echo "✅ Webhook receiver sẵn sàng!"
    echo ""
